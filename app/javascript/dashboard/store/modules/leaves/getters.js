export const getters = {
  ...
  getPendingLeaves: state => {
    return Object.values(state.records).filter(leave => leave.status === 'pending');
  },
  
  getApprovedLeaves: state => {
    return Object.values(state.records).filter(leave => leave.status === 'approved');
  },
  
  getRejectedLeaves: state => {
    return Object.values(state.records).filter(leave => leave.status === 'rejected');
  },
  
  getLeavesByAgent: state => agentId => {
    return Object.values(state.records).filter(leave => leave.agent_id === agentId);
  },
